...
<div class={actif == 3 ? 'wrapp show' : 'wrapp'}>
	...
	<button
		type="button"
		onclick={() => (entreprises = api.callApi(naf, selectedDept, selectedTypeEnt,selectedNbSalarie, page, 20))}
		>Appel</button
	>
	<div class="container">
		<div class="col">
			...
			<p>Tranche nombre de salariés :</p>
			<div class="inputs" >
			...
			<h3>
				...
				{#if entreprises}
					...
					{:then entreprise}
						...
						type="button"
						onclick={() =>
							(entreprises = api.callApi(naf, selectedDept, selectedTypeEnt,selectedNbSalarie, page == 1 ? page=1 : --page, 20))}
					>
						{'<'}
					</button>
					<button
						type="button"
						onclick={() =>
							(entreprises = api.callApi(naf, selectedDept, selectedTypeEnt,selectedNbSalarie, page == maxPage ? page = maxPage : ++page, 20))}
						>{'>'}</button
					>

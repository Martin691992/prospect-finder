			...
			<h3>
				...
				{#if entreprises}
					...
					{:then entreprise}
						...
						type="button"
						onclick={async () =>
							(entreprises = await api.callApi(selectedNaf, selectedDept, selectedTypeEnt, page == 1 ? page=1 : --page, 20))}
					>
						{'<'}
					</button>
					<button
						type="button"
						onclick={async() =>
							(entreprises = await api.callApi(selectedNaf, selectedDept, selectedTypeEnt, page == maxPage ? page = maxPage : ++page, 20))}
						>{'>'}</button
					>
